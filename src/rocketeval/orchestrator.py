...
from .models import DebateTurn, FactorCheck, ParsedAnswerScript, ReviewerAssessment, SupremeReview
...
class EvaluationOrchestrator:
    ...
    def _factor_specialist_reviews(self, script: ParsedAnswerScript) -> list[FactorCheck]:
        ...
        factor_by_name = {factor.name: factor for factor in script.factors}
        for factor_name, agent_model in self.model_config.factor_specialists.items():
            if factor_name not in factor_by_name:
                continue
            factor = factor_by_name[factor_name]
            payload = self.provider.complete_json(
                agent_model=agent_model,
                prompt=factor_review_prompt(script, factor_name=factor.name, factor_weight=factor.weight, factor_description=factor.description),
            )
            score = max(0.0, min(float(payload.get("score", 0.0)), float(factor.weight)))
            checks.append(
                FactorCheck(
